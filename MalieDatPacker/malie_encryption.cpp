...
static constexpr int ValueMask = (1 << BitLen) - 1;
...
static void rotateEnc(uint32_t* data, const MalieKey& rotateKey, uint32_t offset) {
...
static void encrypt_block(const MalieKey& keytable, unsigned char* block, int offset) {
	offset >>= 4;
	auto data32 = reinterpret_cast<uint32_t*>(block);
...
void MalieEncryption::encrypt(unsigned char* data, unsigned int data_length, unsigned int offset) const
{
...
void MalieEncryption::decrypt(unsigned char* data, unsigned int data_length, unsigned int offset) const
{
	//Round 1
	uint8_t szBlock = offset & 0xF;
	uint8_t xorKey = data[szBlock];
	...
	//Round 2
	uint32_t* pDecBuffer = (uint32_t*)data;
	uint8_t shrOffset = offset >> 4;
